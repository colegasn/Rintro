...
The latest version of R available as of the time of this writing is R-4.3.1. You can install R on your computer by going to <https://cran.r-project.org/> and selecting the R version that is appropriate for your operating system.

![The Comprehensive R Archive Network (CRAN) website for downloading R.](r_site.png){fig-align="center"}
...
This should then take you to a splash page where you can install the version of R specific to your operating system. Click **Download R-4.3.1** to download the executable file.

![The Download R page for the Windows operating system.](r_site3.png){fig-align="center"}

Go to your downloads folder or wherever you view files where you download from the internet and look for the `R-4.31.exe` file tailored for your operating system. Open the executable file to begin the installation process. Follow the on-screen prompts to install R on your computer.
...
![The posit site for downloading RStudio.](rstudio_site.png){fig-align="center"}
...
You can highlight multiple lines of code and run them all at once as well. Optionally, you can run lines of code by using the keyboard shortcut **CTRL + ENTER** (or equivalent). To get started, here is some simple lines of code:
...
First run these two lines of codes to download the new_pas.R and new_pat.R files:

```{r download2}
# Download the new_pas.R file from GitHub
download.file("https://raw.githubusercontent.com/geomarker-io/purple_air_data_in_R/main/new_pas.R", destfile = "new_pas.R")

# Download the new_pat.R file from GitHub
...
![Create a new R project within RStudio.](Rproject1.png){alt="Create a new R project within RStudio." fig-align="center" width="400"}
...
![Select a folder that already exists to house the R project.](Rproject4.png){alt="Select a folder that already exists to house the R project." fig-align="center" width="400"}
:::

Select New Directory from the message box. This brings up a list of various project types. Select **New Project**.

![Click **New Project** to create a new R project.](Rproject2.png){alt="Click New Project to create a new R project." fig-align="center" width="400"}
...
![Type in the project name and select where to create the folder for the project.](Rproject3.png){alt="Type in the project name and select where to create the folder for the project." fig-align="center" width="400"}
...
To install the `dplyr`, `ggplot2`, `devtools`, and `MazamaCoreUtils` packages, simply use the `install.packages()` function below.
...
Some R package depend on other R packages (called **dependencies**). Packages that depend on other packages will install these extra packages automatically if they are not already installed in the users library. As an example, the AirSensor2 package requires the `MazamaCoreUtils` package to be already installed:
...
::: callout-caution
Some R packages that depend on other R packages in order to function properly. These extra dependencies are usually installed automatically if they are not present in the users library.
:::

You can view all the R packages installed on your computer by clicking on the **Packages** tab in RStudio on the Output Pane.

::: callout-tip
R already comes with its own packages (e.g., data sets, graphics, etc.). You do not need to install these packages, as they are already pre-installed when R is first installed.
:::
...
![When prompted with a list of packages to update, type `1` and press **ENTER**.](AirSensor2.png){alt="When prompted with a list of packages to update, type 1 and press ENTER." fig-align="center" width="450"}
